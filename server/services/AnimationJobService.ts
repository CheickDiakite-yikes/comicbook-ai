...
type AnimationJobUpdateWithMetadata = AnimationJobUpdate & {
  metadata?: Record<string, unknown>;
};

type AnimationJobUpdateInput = Omit<AnimationJobUpdateWithMetadata, 'status'> & {
  status?: AnimationJobStatus;
};

function deriveProgress(
  update: AnimationJobUpdateWithMetadata,
  existing?: AnimationJobRecord,
): AnimationJobRecord['progress'] | undefined {
  if (update.progress) {
...
export class AnimationJobService {
  ...
  queueJob(update: AnimationJobUpdateInput): AnimationJobRecord {
    return this.pushUpdate({ ...update, status: 'queued' });
  }

  updateJobStatus(update: AnimationJobUpdateWithMetadata): AnimationJobRecord {
    return this.pushUpdate(update);
  }

  pushUpdate(update: AnimationJobUpdateWithMetadata): AnimationJobRecord {
...
export const animationJobService = new AnimationJobService();
