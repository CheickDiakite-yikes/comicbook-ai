...
export class Veo3JobService {
  ...
  private async handleOperationSuccess(
    ...
    if (versionPayloads.length === 0) {
      await this.markFailure(request, 'No video results returned by Veo3', polledAt, request.retryCount + 1);
      return;
    }
    ...
    this.logger.info('Veo3 job completed successfully', {
      requestId: request.id,
      panelId: request.panelId,
      generatedCount: versionPayloads.length,
    });
  }
  ...
  private async handleOperationFailure(
    ...
    const message = this.getOperationErrorMessage(operation) ?? 'Unknown Veo3 failure';
    await this.handleOperationError(request, new Error(message), polledAt);
  }
